...
import { useSesion } from 'next-auth/react';
...
function PromptCard({ post, handleTagClick, handleEdit, handleDelete }) {
    ...
    return (
        <div className='prompt_card'>
            ...
            <p className="my-4 font-satoshi text-sm text-gray-700 truncate">{post.prompt}</p>
            <p className="font-inter text-sm blue_gradient cursor-pointers" onClick={() => handleTagClick && handleTagClick(post.tag)}>{post.tag}</p>
        </div>
    )
}
